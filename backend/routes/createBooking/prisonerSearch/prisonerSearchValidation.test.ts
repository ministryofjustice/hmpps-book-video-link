...
describe('prisoner search validation', () => {
  ...
  it('should error when only first name entered, with whitespace', async () => {
    expect(videolinkPrisonerSearchValidation.validate({ firstName: '   ' })).toEqual([
      { href: '#lastName', text: "You must search using either the prisoner's last name or prison number" },
    ])
  })

  it('should error when only last name entered, with whitespace', async () => {
    expect(videolinkPrisonerSearchValidation.validate({ lastName: '   ' })).toEqual([
      { href: '#lastName', text: "You must search using either the prisoner's last name or prison number" },
    ])
  })

  it('should error if only prison number entered, with whitespace', async () => {
    expect(videolinkPrisonerSearchValidation.validate({ prisonNumber: '  ' })).toEqual([
      { href: '#lastName', text: "You must search using either the prisoner's last name or prison number" },
    ])
  })
  it('should error if both last name and prison number are whitespace only', async () => {
  ...
  it('should error if prison number has 7 charcters including some are whitespaces ', async () => {
    expect(videolinkPrisonerSearchValidation.validate({ prisonNumber: ' A12  ' })).toEqual([
      { text: 'Enter a prison number using 7 characters in the format A1234AA', href: '#prisonNumber' },
    ])
  })
